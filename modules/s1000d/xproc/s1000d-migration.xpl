...
<p:declare-step
    ...
    name="s1000d-migration"
    type="sgproc:s1000d-migration"
    version="3.0">
    
    <p:documentation>
        <p>This is the wrapper XProc pipeline for converting well-formed XML to valid XML</p>
    </p:documentation>
    ...
    <p:input port="sch" select="'../sch/s1000d-checks.sch'"/>
    ...
    <p:for-each name="loop-doctypes">
        <p:with-input select="//doctype[@root!='' and @include='true']" pipe="doctypes@s1000d-migration"/>
        ...
        <sgproc:validate-convert name="run" p:message="Processing {$root} documents with include filter {$include-filter}">
            ...
            <p:with-input port="sch" pipe="sch@s1000d-migration"/>
